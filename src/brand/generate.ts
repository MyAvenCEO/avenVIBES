...
export function createGenerator(brand: Brand) {
	...
	/**
	 * The dark theme, emitted LAST.
	 *
	 * Order decides this, not specificity: `[data-theme="dark"]` and `:root` both
	 * score (0,1,0), so whichever comes second wins. The dark block used to sit
	 * between the surfaces and the roles, which meant every role declared after
	 * it — and that is all of them — beat its own dark override on a page that
	 * stamped `data-theme` on `<html>`.
	 *
	 * It went unnoticed because this brand scopes dark to a SECTION rather than
	 * the document: `:root` does not match a div, so on `#ceobrand` only the dark
	...
	function darkBlock(surfaces: TokenMap, scope?: string): string[] {
		...
		if (!dark.length && !Object.keys(roleOverrides).length) return []
		return [
			'',
